...
TEST_CASE("fail compress - pointer, debug throws int overflow") {
  ...
  CHECK_THROWS_WITH(gzip::compress(pointer, l),
                    Catch::Contains("size arg is too large to fit into unsigned int type"));
}
#endif
...
TEST_CASE("fail decompress - pointer, debug throws int overflow") {
  ...
  CHECK_THROWS_WITH(gzip::decompress(compressed_pointer, l),
                    Catch::Contains("size arg is too large to fit into unsigned int type x2"));
}
#endif
...
TEST_CASE("round trip compression - gzip") {
  ...
  SUBCASE("compression level - invalid") {
    int level = 99;

    CHECK_THROWS(gzip::compress(data.data(), data.size(), level));
  }

  SUBCASE("no compression") {
  ...
  SUBCASE("compression level -- min to max") {
